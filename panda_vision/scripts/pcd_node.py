...
rospy.init_node('get_pc', anonymous=True)
rospy.loginfo("GETTING PONT_CLOUD")
rospy.Subscriber("/points2", PointCloud2)
rospy.spin()


################## reading image from the bag file #########################
def pcd_maker():
    pcd = o3d.io.read_point_cloud("/home/rnm/Desktop/11/1656951005774891.pcd" )           #("/home/rnm/Desktop/pcd_data/1590496721.382030864.pcd" )  
    ...
    pcd = o3d.io.read_point_cloud("/home/rnm/Desktop/11/cropped_1.ply")     #("/home/rnm/catkin_ws/src/panda_vision/scripts/cropped_03.ply")
...
if __name__ == "__main__":
    ...
    target.add_geometry(source)
    target.run()
    target.destroy_window()
    tgt_pts= []
    tgt_pts= target.get_picked_points()  # shift+lmb to select target point 
    
    a=np.shape(result_icp.transformation) 
    print(a)

    tgt_pts=np.reshape(4,1, order="F")
    print(tgt_pts)
 
    print(np.dot(a,np.transpose(tgt_pts)))
